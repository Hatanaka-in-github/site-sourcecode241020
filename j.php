...
<body>
<form class="container" action="j.php" method="post" enctype="multipart/form-data" autocomplete="off">
    ...
    foreach ($reader->getSheetIterator() as $sheet)
    {
     foreach ($sheet->getRowIterator() as $row)
     {
      ...
      if(!empty($array[0]) && $i > 0 )
      {
                ...
                $NGAmount = 1 ;
                $FromProcess = "/".$array[5]."/" ;
                if(!empty($array[6])){$ItemNum = $array[6] ; } ;
                if(!empty($array[7])){$OrderNum = $array[7] ; } ;
                $Notice = $array[8] ;
                $Member = ($_POST["入力者"]) ;
                if(file_exists('admin/p/all.php')){include('admin/p/all.php');};


       if(!empty($PName) && !empty($IODATE) && !empty($Serial) && !empty($FromProcess) && !empty($Notice))
        {
          ...
          if($FromProcess === "/".$NoValidate[0]."/" || 
             ...
             $FromProcess === "/".$NoValidate[2]."/" )
            {
              ...
              {if($value >= 1 && !empty($MinusAdminNum[0]))
                {    
                  ...
                  try{
                    ...
                    values ('{$IDate}','{$PName}','{$PNumber}','{$MinusAdminNum[0]}',-1*{$NGAmount},'{$Serial_NoValid[0]}','{$FromProcess}','{$Member}','{$SerialProcess}',2);");

                    $NGSerialProcess = $PName.'NG'.$Serial.$FromProcess.$PNumber ;
                    $db->QuerySql("insert into `process_progress`
                    (入出庫日,入力日,品名,品番,管理番号,シリアル,受工程,損品数,規格1,規格2,備考,入力者,シリアル工程,工程キー,exe_flg)
                    values ('{$IODATE}','{$IDate}','{$PName}','{$PNumber}','{$AdminNum}','{$Serial}','{$FromProcess}','{$NGAmount}',
                    '{$ItemNum}','{$OrderNum}','{$Notice}','{$Member}','{$NGSerialProcess}','{$FromProcess}',0);");

                    $db->QuerySql("insert into `NGtable`
                    (入力日,入力者,入出庫日,品名,管理番号,型番,シリアル,工程,規格1,規格2,NG理由,exe_flg)
                    values ('{$IDate}','{$Member}','{$IODATE}','{$PName}','{$AdminNum}','{$PNumber}','{$Serial}','{$FromProcess}','{$ItemNum}','{$OrderNum}','{$Notice}',0);");
...
/*
                 if( $FromProcess == "/".$TYPE1check[0]."/"  ||
                           ...
                           {if($value >= 1 && !empty($NGAmount) && !empty($MinusAdminNum_TYPE1check[0]))
                              {
                                ...
                                try{
                                    ...
                                    values ('{$IDate}','{$PName}','{$PNumber}','{$MinusAdminNum_TYPE1check[0]}',-1*{$NGAmount},'{$Serial}','{$FromProcess}','{$Member}','{$ItemNumCheck[0]}','{$SerialProcess}',2);");
            
                                    $NGSerialProcess = $PName.'NG'.$Serial.$FromProcess.$PNumber ;
                                    $db->QuerySql("insert into `process_progress`
                                    (入出庫日,入力日,品名,品番,管理番号,シリアル,受工程,損品数,規格1,規格2,備考,入力者,シリアル工程,工程キー,exe_flg)
                                    values ('{$IODATE}','{$IDate}','{$PName}','{$PNumber}','{$MinusAdminNum_TYPE1check[0]}','{$Serial}','{$FromProcess}','{$NGAmount}',
                                    '{$ItemNumCheck[0]}','{$OrderNum}','{$Notice}','{$Member}','{$NGSerialProcess}','{$FromProcess}',0);");
            
                                    $db->QuerySql("insert into `NGtable`
                                    (入力日,入力者,入出庫日,品名,管理番号,型番,シリアル,工程,規格1,規格2,NG理由,exe_flg)
                                    values ('{$IDate}','{$Member}','{$IODATE}','{$PName}','{$MinusAdminNum_TYPE1check[0]}','{$PNumber}','{$Serial}','{$FromProcess}','{$ItemNumCheck[0]}','{$OrderNum}','{$Notice}',0);");
                   ...
                   foreach($Sum0ornot[0] as $key => $value)
                          {if($value >= 1 && !empty($NGAmount) && !empty($MinusAdminNum[0]))
                             {
                              ...
                              try{
                                  ...
                                  $db->QuerySql("insert into `process_progress`
                                  (入力日,品名,品番,管理番号,数量,シリアル,受工程,入力者,規格1,規格2,シリアル工程,exe_flg)
                                  values ('{$IDate}','{$PName}','{$PNumber}','{$MinusAdminNum[0]}',-1*{$NGAmount},'{$Serial}','{$FromProcess}','{$Member}','{$TYPE1Check[0]}','{$TYPE2Check[0]}','{$SerialProcess}',2);");
                  ...
                  if( $FromProcess == "/".$TYPE1check[0]."/" || $FromProcess == "/".$TYPE1check[1]."/" || $FromProcess == "/".$TYPE1check[2]."/" )
                     {
                      if(!is_dir('./TYPE1/'.$FromProcess.'/')){ mkdir( './TYPE1/'.$FromProcess.'/' ); } ; 
                      if(!file_exists('./TYPE1/'.$FromProcess.'/'.$PName.'_'.$PNumber.'&'.$AdminNum.'@'.$ItemNum2.';')){ touch('./TYPE1/'.$FromProcess.'/'.$PName.'_'.$PNumber.'&'.$AdminNum.'@'.$ItemNum2.';'); } ;
     
                     }elseif( $FromProcess == "/".$Serialcheck[0]."/" || $FromProcess == "/".$Serialcheck[1]."/" || $FromProcess == "/".$Serialcheck[2]."/" )
                     {
                       if(!is_dir('./TYPE1/'.$FromProcess.'/')){ mkdir( './TYPE1/'.$FromProcess.'/' ); } ; 
                       if(!file_exists('./TYPE1/'.$FromProcess.'/'.$PName.'_'.$PNumber.'&'.$AdminNum.'@'.';'.$Serial )){ touch('./TYPE1/'.$FromProcess.'/'.$PName.'_'.$PNumber.'&'.$AdminNum.'@'.';'.$Serial); } ;
                     }else{
                      if(!is_dir('./TYPE1/'.$FromProcess.'/')){ mkdir( './TYPE1/'.$FromProcess.'/' ); } ; 
                      if(!file_exists('./TYPE1/'.$FromProcess.'/'.$PName.'_'.$PNumber.'&'.$AdminNum.'@')){ touch('./TYPE1/'.$FromProcess.'/'.$PName.'_'.$PNumber.'&'.$AdminNum.'@'); } ;
                     };
...
/*
         if(!empty($_POST["品名"]) && isset($_POST['product-group']) && file_exists($_POST['product-group']."-pg.php")){
...
*/
/*
         }elseif(!empty($_POST["品名"]) && file_exists("./p/".$_POST["品名"].".php")){include("./p/".$_POST["品名"].".php");
*/
        $LIST0 = "/".$LIST[0]."/";
	      $LIST1 = "/".$LIST[1]."/";
	      $LIST2 = "/".$LIST[2]."/";
        ...
        try{
              ...
              /*在庫なので、値は正でなくてはならない。負の数になる場合は入力内容のexe_flgを1にして集計から除外し、在庫表を示す。*/
               ...
               if($PSum1 < 0 || $PSum2 < 0 || $PSum3 < 0 || $PSum4 < 0 || $PSum5 < 0 || $PSum6 < 0
			           ...
			           ){ echo $r."行目_"."$PName"."の在庫がマイナスしています。処理を中断します。";
