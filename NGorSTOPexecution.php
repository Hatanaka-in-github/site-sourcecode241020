...
if(!empty($_POST['hidden-parameter3-2']) && !empty($_POST['record-id'])){
...
}elseif(!empty($_POST['hidden-parameter3-3']) && !empty($_POST['record-id2'])){
 ...
 $GoPNum = $db->SelectSumAllarray("SELECT 品番 FROM process_progress WHERE exe_flg != 1 and AUTO_ID = '{$_POST['record-id2']}';");
 ...
 $GoType = $db->SelectSumAllarray("SELECT 規格1 FROM process_progress WHERE exe_flg != 1 and AUTO_ID = '{$_POST['record-id2']}';");
...
$GoSerialProcess = $GoName[0].'ST'.$GoSerial[0].$GoProcess[0].$GoPNum[0] ;
 $db->QuerySql("insert into `process_progress` (入力日,品名,品番,シリアル,管理番号,保留数,受工程,規格1,備考,シリアル工程,入力者,exe_flg)
 values ('{$IDate}','{$GoName[0]}','{$GoPNum[0]}','{$GoSerial[0]}','{$Goadnum[0]}',-1,'{$GoProcess[0]}','{$GoType[0]}','{$OKReason}','{$GoSerialProcess}','{$Member}',2);");

 $GoSerialProcess = $GoName[0].$GoSerial[0].$GoProcess[0].$GoPNum[0] ;
 $db->QuerySql("insert into `process_progress` (入力日,品名,品番,数量,シリアル,管理番号,受工程,規格1,備考,シリアル工程,入力者,exe_flg) 
 values ('{$IDate}','{$GoName[0]}','{$GoPNum[0]}',1,'{$GoSerial[0]}','{$Goadnum[0]}','{$GoProcess[0]}','{$GoType[0]}','{$OKReason}','{$GoSerialProcess}','{$Member}',0);");
...
*/
...
}elseif(empty($Q)){ echo "指定のレコードIDに保留数はありません。" ; $Q = 1; } ;


}elseif(!empty($_POST['hidden-parameter4-4']) && !empty($_POST['record-id3'])){
 /*$db->QuerySql("UPDATE process_progress SET exe_flg = 1 where 入力日 = '{$DelDate[0]}' and 入力者 = '{$DelMem[0]}';"); */
 ...
 unset($_POST['hidden-parameter4-4']);  }; 
   ...
   echo '<br><br>NGorSTOPexecution.php:4〜79_処理に失敗しました。管理者にお問合せ下さい。<br><br>'; 
                            };
                            ?>